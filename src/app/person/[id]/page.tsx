...
import type { CreditTab, PersonCreditsResponse, PersonDetails } from "@/types";
...
export default function Page() {
  ...
  const {
    ...
    error: personError,
  } = useFetch<PersonDetails, [string]>(fetchPersonDetails, id);

  const {
    data: credits,
    loading: creditsLoading,
    error: creditsError,
  } = useFetch<PersonCreditsResponse, [string]>(fetchPersonCredits, id);
  ...
  const handleTabChange = (tab: CreditTab) => {
    setActiveTab(tab);
  ...
  if (personError || creditsError) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-bg text-red-500 text-xl p-8 text-center">
        Error: {personError || creditsError || "Something went wrong"}
      </div>
    );
  }
        ...
        <section className="container mx-auto px-4 md:px-8 lg:px-12 py-6">
          ...
          <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
            ...
            <div className="flex gap-1 bg-zinc-900/80 p-1 rounded-lg border border-zinc-800">
              ...
              ].map((tab: { key: CreditTab; label: string; count: number }) => (
                <button
                  key={tab.key}
