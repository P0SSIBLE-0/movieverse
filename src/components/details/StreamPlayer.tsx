...
import type { DropdownOption, StreamPlayerProps } from '../../types';
...
const StreamPlayer = ({
  ...
  const serverOptions = useMemo<DropdownOption<string>[]>(
    () => STREAMING_PROVIDERS.map((p) => ({ value: p.id, label: p.name })),
    []
  );

  const seasonOptions = useMemo<DropdownOption<number>[]>(() => {
  ...
  const episodeOptions = useMemo<DropdownOption<number>[]>(
    () =>
