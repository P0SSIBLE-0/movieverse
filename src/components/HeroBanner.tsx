...
import type { MediaItem, MediaType, PaginatedResponse, WatchlistSourceItem } from "../types";
...
function HeroBanner() {
  ...
  useEffect(() => {
    ...
    fetchTrending("movie", "week") // Fetch weekly trending movies for hero
      .then((res) => {
        const trendingData = res.data as PaginatedResponse<MediaItem>;
        setHeroSlides(trendingData.results.slice(0, 5));
        setLoading(false);
      })
      .catch((err) => {
      ...
      >
        {heroSlides.map((slide) => {
          const resolvedType: MediaType =
            slide.media_type === "tv" || slide.media_type === "anime"
              ? slide.media_type
              : "movie";
          const releaseValue = slide.release_date || slide.first_air_date;
          ...
          <SwiperSlide key={slide.id} className="relative">
            ...
            <div className="absolute bottom-0 left-0 p-4 md:p-8 lg:p-12 text-white w-full md:w-2/3 lg:w-1/2">
              ...
              <div className="flex items-center space-x-4 mb-2 md:mb-4 text-sm md:text-base text-brand-text-secondary">
                ...
                <span>{releaseValue ? new Date(releaseValue).getFullYear() : "N/A"}</span>
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  className={`backdrop-blur-sm px-4 py-2 md:py-3 rounded-full font-semibold flex items-center justify-center md:space-x-2 transition-all duration-300 text-sm md:text-base cursor-pointer  ${isInWatchlist(slide.id, resolvedType)
                ...
                >
                  {isInWatchlist(slide.id, resolvedType) ? (
